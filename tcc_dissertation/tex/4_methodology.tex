...
During train phase, all models listed in \ref{sec_environment} were rab with batch size of 64 samples. With 50 epochs per train and 15 early stop behavior, all models were trained with the following optimizers configuration:
\begin{itemize}
...
\end{itemize}
All optimizers hyperparams were choosen based on another works \ref{chap:related}, for a measurable comparasions with other results, as explored in \ref{chap:results}. Also, when applied, scheduler was configured with step size of 5 and $\gamma$ of $0.1$.


\section{Procediments}
\label{sec_procediments}
All development was built in \gls{VSCODE} \gls{IDE} the "Colab" extension, where all dependecies were easily installed with a Jupyter Notebook comand. For the dataset, as all data is kept inside the cloud service storage, the datasets were downloaded through their respective \gls{API}.

After downloading images necessary, it was necessary convert them to a single csv file. For all datasets, the csv was generated with two columns: path and label. In the first one, was the path the program would need to course to get the image, while the second indicats the image class. For training, label need to be a numeric value, so the following transformation was made:
\begin{itemize}
    \item 1: cancer image
...
With the respective \gls{CSV} file, the samples were splitted in validation, test and train and them transformed 224 x 224 pixels for normalization and train efficiency \cite{DBLP:conf/nips/KrizhevskySH12}.
